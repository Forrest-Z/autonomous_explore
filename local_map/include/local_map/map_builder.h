...
class MapBuilder
{
  ...
  private:
    ...
    void updatePointOccupancy(bool use_bayes, bool occupied, size_t idx, vector<int8_t>& occupancy, vector<double>& log_odds) const;

    /** Update occupancy and log odds for a list of a points
    */
    inline void updatePointsOccupancy(bool use_bayes, bool occupied, const vector<size_t>& indexes, vector<int8_t>& occupancy, vector<double>& log_odds)
    {
