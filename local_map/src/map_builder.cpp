 ...
 */
void MapBuilder::updatePointOccupancy(bool use_bayes, bool occupied, size_t idx, vector<int8_t>& occupancy, vector<double>& log_odds) const
{
...
//    last_ymap_ = lround(yinit_ / map_.info.resolution);
  ...
  ROS_INFO_STREAM("scan yaw " << theta << '\n');
...
//  map_.data.assign(map_.info.width * map_.info.height, -1);
  ...
  for (size_t i = 0; i < scan.ranges.size(); ++i)
  {
    ...
    // add by zwk for considering unknown area is free area
    if((scan.ranges[i] == scan.range_max) && obstacle_in_map) {
