...
namespace exploration {
    ...
    struct Weights {
        Weights() : explCells(1.0), angDist(1.0), robotGoalDist(1.0), driveability(1.0), edgeDetected(1.0) {
        }

        Weights(double expl, double ang, double dist, double driveability, double edge) : explCells(expl), angDist(ang),
                                                                                          robotGoalDist(dist),
