...
namespace exploration {
    enum Status {
        SUCCESS, NO_GOAL, ERROR, NOT_IMPLEMENTED
    };
    enum DrivabilityClasses {
        VISIBLE, OBSTACLE = 2, EXPLORED, UNKNOWN, GOAL_CELL
    };
    ...
    struct Pose {
        ...
        Pose(Eigen::Vector3d vec) : x(vec[0]), y(vec[1]), theta(0.0) {
        }

        Pose(double a, double b, double c) : x(a), y(b), theta(c) {}
    ...
    struct GridPoint {
        ...
        GridPoint(Eigen::Vector3d vec) : x(vec[0]), y(vec[1]), distance(0) {
        }
    ...
    public:
        ...
        bool setData(GridPoint p, char v) {
            if (!isAllocated || p.x < 0 || p.y < 0 || p.x >= (int) width || p.y >= (int) height) {
                return false;
            }
